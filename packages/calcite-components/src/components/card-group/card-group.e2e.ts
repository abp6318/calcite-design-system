...
describe("calcite-card-group", () => {
  describe("renders", () => {
    renders("<calcite-card-group><calcite-chip></calcite-chip></calcite-card-group>", {
      display: "flex"
    });
  });
  ...
  describe("disabled", () => {
    disabled("<calcite-card-group><calcite-chip></calcite-chip></calcite-card-group>", {
      focusTarget: "child"
    });
  });
...
async function assertSelectedItems(page: E2EPage, { expectedItemIds }: SelectedItemsAssertionOptions): Promise<void> {
  ...
  const selectedItemIds = await page.evaluate(() => {
    const chipGroup = document.querySelector<HTMLCalciteChipGroupElement>("calcite-card-group");
